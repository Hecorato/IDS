...
with st.container(border=True):
    ...
    if df_conectores.empty:
        ...
        if len(cuentas_reincidentes) == 0:
            ...
            df_conectores['Tecnico_anterior'] = df_conectores.groupby('Cuenta')['Nombre tecnico'].shift(1)
            df_conectores['Fecha_anterior'] = df_conectores.groupby('Cuenta')['Fecha termino'].shift(1)
            df_conectores['Empresa_anterior'] = df_conectores.groupby('Cuenta')['Empresa(proveedor)'].shift(1)
            df_fallas = df_conectores[df_conectores['Reincidencia_60d']].dropna(subset=['Tecnico_anterior'])
            ...
            df_top_tecnicos = (
                ...
                .agg(
                    Reincidencias=('OS', 'count'),
                    Cuentas=('Cuenta', 'nunique'),
                    Empresa=('Empresa_anterior', lambda x: x.mode()[0] if x.notna().any() else 'N/A')
                )
                .reset_index()
                .sort_values('Reincidencias', ascending=True)
                .tail(10)
            ...
            df_top_tecnicos['Etiqueta'] = df_top_tecnicos['Tecnico_anterior'] + ' (' + df_top_tecnicos['Empresa'] + ')'

            colores_tec = {
                t: '#e63946' if i == len(df_top_tecnicos) - 1 else '#1f77b4'
                for i, t in enumerate(df_top_tecnicos['Etiqueta'])
            }

            fig_tecnicos = px.bar(
                df_top_tecnicos, x='Reincidencias', y='Etiqueta', orientation='h',
                text='Reincidencias', color='Etiqueta', color_discrete_map=colores_tec
            )
            fig_tecnicos.update_traces(textposition='outside')
            fig_tecnicos.update_layout(
                height=80 + 10 * 35, xaxis_title='Instalaciones que tuvieron retrabajo (60 dias)', yaxis_title='',
            ...
            if evento_tec and evento_tec.selection and evento_tec.selection.points:
                etiqueta_sel = evento_tec.selection.points[0]['y']
                tecnico_sel = df_top_tecnicos[df_top_tecnicos['Etiqueta'] == etiqueta_sel]['Tecnico_anterior'].values[0]
                ...
                with st.container(border=True):
                    ...
                    df_cuentas_tec = df_cuentas_tec[
                        ...
                        'Nombre tecnico': 'Tecnico_que_regreso',
                        'Fecha_anterior': 'Primera_atencion'
                    }).sort_values('Fecha termino', ascending=False)
