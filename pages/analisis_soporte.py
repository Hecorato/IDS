...
def cargar_join():
    ...
    df_puertos['es_FH'] = df_puertos['Terminal Type'].str.contains('FH', na=False).astype(int)
    ...
    df['es_FH'] = df['es_FH'].fillna(0).astype(int)
...
with tab_detalle:
    cols = ['CUENTA', 'FECHA CREACION', 'NIVEL2', 'ESTATUS', 'OLT_NCE', 'FSP', 'QR', 'Estado_ONT', 'Modelo', 'es_FH', 'CLUSTER INSTALACION']
